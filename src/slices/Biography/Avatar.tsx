  ...
  useEffect(() => {
    let ctx = gsap.context(() => {
      ...
      window.onmousemove = (e) => {
        ...
        let distanceFromCenter = 1 - Math.abs(componentPercent.x);
        ...
        gsap
          ...
          .to(
            ".highlight",
            {
              opacity: distanceFromCenter - 0.7,
  ...
  return (
    <div ref={component} className={clsx("relative h-full w-full", className)}>
      <div className="avatar aspect-square overflow-hidden rounded-3xl border-2 border-slate-700 opacity-0">
        <PrismicNextImage
          field={image}
          className="avatar-image h-auto w-full object-fill"
          imgixParams={{ q: 90 }}
        />
        <div className="highlight absolute inset-0 z-0 hidden w-full scale-110 bg-gradient-to-tr from-transparent via-slate-300 to-transparent opacity-0 md:block"></div>
      </div>
    </div>
  );
}
